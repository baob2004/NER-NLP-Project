...
train.py
========
Train XLM-RoBERTa-base NER cho bản án ma túy với dữ liệu ít/lệch nhãn.

Ưu điểm:
    - Không dùng HuggingFace Trainer -> tránh lỗi version transformers.
    - Không dùng offset_mapping.
    - Tokenize thủ công từng word, tương thích với XLM-RoBERTa/SentencePiece và giữ nhãn BIO theo word.
...
Chạy:
    python train_xlmr.py --data ../data/annotated/drug.jsonl --output ../models/xlmr-ner-weighted --epochs 40 --batch 4 --lr 2e-5

Chỉ xuất BIO debug:
    python train_xlmr.py --data ../data/annotated/drug.jsonl --output ../models/xlmr-ner-weighted --bio-only
"""
...
DEFAULT_OUTPUT = Path(__file__).resolve().parent.parent / "models" / "xlmr-ner-weighted"
...
def main():
    parser = argparse.ArgumentParser(description="Train XLM-RoBERTa-base weighted NER 8 nhãn cho bản án ma túy")
