...
Chạy:
    python train_ner_model.py --data ../data/annotated/drug.jsonl --output models/phobert-drug-ner --epochs 40 --batch 4 --lr 3e-5

Chỉ xuất BIO debug:
    python train_ner_model.py --data ../data/annotated/drug.jsonl --output models/phobert-drug-ner --bio-only
"""
...
from ner_evaluator import evaluate_model, print_results


DEFAULT_DATA = Path(__file__).resolve().parent.parent / "data" / "annotated" / "drug.jsonl"
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "models" / "phobert-drug-ner"
